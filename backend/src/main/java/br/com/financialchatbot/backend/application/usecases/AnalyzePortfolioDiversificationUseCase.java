...
public class AnalyzePortfolioDiversificationUseCase {
    ...
    public record Output(byte[] imageBytes) {}
    ...
    public Output execute(Input input) {
        ...
        if (portfolio.getAssets().isEmpty()) {
            return new Output(new byte[0]);
        }

        byte[] imageResult = analysisGateway.analyzeDiversification(portfolio.getAssets());

        return new Output(imageResult);
    }
}